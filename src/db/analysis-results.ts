...
export function createAnalysisResultComment(result: Omit<AnalysisResultComment, 'id'>): AnalysisResultComment {
  const id = generateId();
  run(
...
export function createAnalysisResultMedia(result: Omit<AnalysisResultMedia, 'id'>): AnalysisResultMedia {
  const id = generateId();
  run(
...
export function listResultsByTask(taskId: string, targetType: 'comment' | 'media', limit = 100): unknown[] {
...
export function getResultById(id: string, targetType: 'comment' | 'media'): unknown | null {
  if (targetType === 'comment') {
    const rows = query<AnalysisResultComment>('SELECT * FROM analysis_results_comments WHERE id = ?', [id]);
    return rows[0] ?? null;
  } else {
    const rows = query<AnalysisResultMedia>('SELECT * FROM analysis_results_media WHERE id = ?', [id]);
    return rows[0] ?? null;
  }
}

export function aggregateStats(taskId: string): Record<string, unknown> {
  const sentimentStats = query<{ sentiment_label: string; cnt: bigint }>(
    'SELECT sentiment_label, COUNT(*) as cnt FROM analysis_results_comments WHERE task_id = ? GROUP BY sentiment_label',
    [taskId]
  );
  const intentStats = query<{ intent: string; cnt: bigint }>(
    'SELECT intent, COUNT(*) as cnt FROM analysis_results_comments WHERE task_id = ? GROUP BY intent',
    [taskId]
  );
  const riskStats = query<{ cnt: bigint }>(
    'SELECT COUNT(*) as cnt FROM analysis_results_comments WHERE task_id = ? AND risk_flagged = true',
    [taskId]
  );
  const total = query<{ cnt: bigint }>(
