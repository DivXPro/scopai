...
export function createMediaFile(file: Omit<MediaFile, 'id' | 'created_at'>): MediaFile {
  const id = generateId();
  const ts = now();
  run(
...
export function listMediaFilesByPost(postId: string): MediaFile[] {
...
export function getMediaFileById(id: string): MediaFile | null {
  const rows = query<MediaFile>('SELECT * FROM media_files WHERE id = ?', [id]);
  return rows[0] ?? null;
}
