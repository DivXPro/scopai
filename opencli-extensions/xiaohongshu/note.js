...
cli({
    ...
    func: async (page, kwargs) => {
      ...
      (() => {
        ...
        const authorPage = document.querySelector('a[href*="/user/profile/"]')
        const authorPageUrl = authorPage ? authorPage.href : ''
        ...
        return { pageUrl: location.href, securityBlock, loginWall, notFound, title, desc, author, authorPageUrl, likes, collects, comments, tags }
      })()
    `);
        ...
        const authorIdMatch = (d.authorPageUrl || '').match(/\/user\/profile\/([a-zA-Z0-9]+)/i);
        const rows = [
            { field: 'note_id', value: noteId },
            { field: 'title', value: d.title || '' },
            { field: 'author', value: d.author || '' },
            { field: 'author_id', value: authorIdMatch ? authorIdMatch[1] : '' },
