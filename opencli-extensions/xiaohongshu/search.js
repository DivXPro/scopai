...
cli({
    ...
    columns: ['rank', 'title', 'author', 'author_id', 'likes', 'published_at', 'is_video', 'cover_image', 'note_id', 'url'],
    func: async (page, kwargs) => {
      ...
      (() => {
        ...
        document.querySelectorAll('section.note-item').forEach(el => {
          ...
          // Extract cover image from the detail link or its child img
          let coverImage = '';
          if (detailLinkEl) {
            const imgEl = detailLinkEl.querySelector('img');
            coverImage = imgEl?.getAttribute('src') || detailLinkEl.getAttribute('src') || '';
          }

          // Detect video: presence of play icon, duration badge, or video-specific classes
          const isVideo = !!(
            el.querySelector('.play-icon, .video-icon, .video-tag, .duration, .video-duration') ||
            el.querySelector('svg[class*="play"], .icon-video')
          );
    ...
    `);
        ...
        return data
            ...
            .map((item, i) => {
            const noteIdMatch = item.url.match(/\/(?:search_result|explore|note)\/([0-9a-f]{24})(?=[?#/]|$)/i);
            const authorIdMatch = item.author_url.match(/\/user\/profile\/([a-zA-Z0-9]+)/i);
            return {
                rank: i + 1,
                ...item,
